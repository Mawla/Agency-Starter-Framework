...
import {Stack, Box, Button, Flex, useToast, Dialog, Text} from '@sanity/ui'
...
export const CopyPaste: ComponentType<any> = (props) => {
  ...
  const document = useFormValue([]) as {_id: string; _type: string; [key: string]: any}
      ...
      {pastedData && dialogOpen && (
        ...
        >
          <Stack>
            <Box padding={4}>
              <Stack space={4}>
                <Text>Are you sure you want to proceed? This will overwrite existing content.</Text>
              </Stack>
            </Box>

            <Stack padding={4}>
              <Flex gap={2}>
                <Button text="Import" icon={ArchiveIcon} mode="ghost" onClick={writePastedData} />
                <Button text="Cancel" mode="ghost" onClick={() => setPastedData(null)} />
              </Flex>
            </Stack>
          </Stack>
        </Dialog>
