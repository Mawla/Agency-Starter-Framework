...
const StylesPanel = (props: ObjectInputProps) => {
  ...
  const handleChange = useCallback(
    (name: string, val: any) => {
      const newValue = {...JSON.parse(JSON.stringify(value || {})), [name]: val}
      ...
      <div>
        {groups.map((group: string) => (
          <Stack space={2} key={group} marginTop={4}>
            ...
            <Grid columns={schemaType.options.columns || 1} marginTop={1} gap={2}>
              {schemaType.options.fields
                ?.filter((x: any) => x.group === group)
                ?.map((item: StyleItemProps) => {
                  return <StyleItem {...item} onChange={handleChange} value={value} />
                })}
            </Grid>
          </Stack>
        ))}

        {Boolean(schemaType.options.fields?.filter((x: any) => !Boolean(x.group)).length) && (
          <Stack space={2}>
            {groups.length > 0 && (
...
const StyleItem = ({name, title, type, options, onChange, value}: StyleItemProps) => (
  <Flex gap={3} align="center">
    ...
    {type === 'boolean' && (
      <Toggle value={value?.[name]} onChange={(value) => onChange(name, value)} />
    )}

    {type === 'string' && (
      <TextInput value={value?.[name]} onChange={(value) => onChange(name, value)} />
    )}
